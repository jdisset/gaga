...
template <typename Iterator> struct future_info {
	typedef typename std::iterator_traits<Iterator>::value_type future_type;
	typedef
	    typename std::result_of<decltype (&future_type::get)(future_type)>::type value_type;
...
template <typename Iterator> inline void wait(Iterator first, Iterator last) {
	for (; first != last; ++first) first->wait();
}
...
inline Result wait_for(Iterator first, Iterator last,
                       const std::chrono::duration<Rep, Period>& timeout_duration,
                       Result result) {
	for (; first != last; ++first) result.push_back(first->wait_for(timeout_duration));
	return result;
}
...
inline std::vector<std::future_status> wait_for(
...
inline Result wait_until(Iterator first, Iterator last,
                         const std::chrono::time_point<Clock, Duration>& timeout_time,
                         Result result) {
	for (; first != last; ++first) result.push_back(first->wait_until(timeout_time));
	return result;
}
...
inline std::vector<std::future_status> wait_until(
    Iterator first, Iterator last,
    const std::chrono::time_point<Clock, Duration>& timeout_time) {
...
template <typename Iterator, typename = typename std::enable_if<
                                 cxxpool::detail::future_info<Iterator>::is_void>::type>
inline void get(Iterator first, Iterator last) {
	for (; first != last; ++first) first->get();
}
...
template <typename Result, typename Iterator,
          typename = typename std::enable_if<
              !cxxpool::detail::future_info<Iterator>::is_void>::type>
inline Result get(Iterator first, Iterator last, Result result) {
	for (; first != last; ++first) result.push_back(first->get());
	return result;
}
...
inline std::vector<typename detail::future_info<Iterator>::value_type> get(
    Iterator first, Iterator last) {
	return cxxpool::get(
	    first, last,
	    std::vector<typename cxxpool::detail::future_info<Iterator>::value_type>{});
}
...
class priority_task {
 public:
	...
	priority_task() : callback_{}, priority_{}, order_{} {}
	...
	void operator()() const { return callback_(); }
...
class thread_pool_error : public std::runtime_error {
 public:
	explicit thread_pool_error(const std::string& message) : std::runtime_error{message} {}
};
...
class thread_pool {
 public:
	...
	explicit thread_pool(std::size_t n_threads) : thread_pool{} {
		if (n_threads > 0) {
	...
	//template <typename Cont, typename F> void for_each(Cont& container, F&& f) {
		//std::vector<std::future<void>> futures;
		//futures.reserve(container.size());
		//for (auto& e : container) futures.push_back(push(std::forward<F>(f), e));
	...
	std::size_t n_threads() const {
		{
			std::lock_guard<std::mutex> task_lock(task_mutex_);
			if (done_) throw thread_pool_error{"n_threads called while pool is shutting down"};
		}
		std::lock_guard<std::mutex> thread_lock(thread_mutex_);
	...
	void set_pause(bool enabled) {
		...
		if (!paused_) task_cond_var_.notify_all();
	}
 ...
 private:
	void worker() {
		for (;;) {
			cxxpool::detail::priority_task task;
			{
				...
				task_cond_var_.wait(task_lock,
				                    [this] { return !paused_ && (done_ || !tasks_.empty()); });
	...
	cxxpool::detail::infinite_counter<typename detail::priority_task::counter_elem_t>
	    task_counter_;
