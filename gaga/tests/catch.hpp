...
// CATCH_CONFIG_CPP11_GENERATED_METHODS : The delete and default keywords for compiler
// generated methods
...
#if defined(CATCH_INTERNAL_CONFIG_CPP11_NULLPTR) &&                                    \
    !defined(CATCH_CONFIG_CPP11_NO_NULLPTR) && !defined(CATCH_CONFIG_CPP11_NULLPTR) && \
    !defined(CATCH_CONFIG_NO_CPP11)
#define CATCH_CONFIG_CPP11_NULLPTR
#endif
#if defined(CATCH_INTERNAL_CONFIG_CPP11_NOEXCEPT) &&                                     \
    !defined(CATCH_CONFIG_CPP11_NO_NOEXCEPT) && !defined(CATCH_CONFIG_CPP11_NOEXCEPT) && \
    !defined(CATCH_CONFIG_NO_CPP11)
...
#define CATCH_CONFIG_CPP11_GENERATED_METHODS
    ...
    !defined(CATCH_CONFIG_CPP11_NO_IS_ENUM) && !defined(CATCH_CONFIG_CPP11_IS_ENUM) && \
    !defined(CATCH_CONFIG_NO_CPP11)
#define CATCH_CONFIG_CPP11_IS_ENUM
#endif
#if defined(CATCH_INTERNAL_CONFIG_CPP11_TUPLE) &&                                  \
    !defined(CATCH_CONFIG_CPP11_NO_TUPLE) && !defined(CATCH_CONFIG_CPP11_TUPLE) && \
    !defined(CATCH_CONFIG_NO_CPP11)
#define CATCH_CONFIG_CPP11_TUPLE
#endif
#if defined(CATCH_INTERNAL_CONFIG_VARIADIC_MACROS) && \
    !defined(CATCH_CONFIG_NO_VARIADIC_MACROS) && !defined(CATCH_CONFIG_VARIADIC_MACROS)
...
#endif
#if defined(CATCH_INTERNAL_CONFIG_CPP11_OVERRIDE) &&                               \
    !defined(CATCH_CONFIG_NO_OVERRIDE) && !defined(CATCH_CONFIG_CPP11_OVERRIDE) && \
    !defined(CATCH_CONFIG_NO_CPP11)
#define CATCH_CONFIG_CPP11_OVERRIDE
#endif
#if defined(CATCH_INTERNAL_CONFIG_CPP11_UNIQUE_PTR) &&                                 \
    !defined(CATCH_CONFIG_NO_UNIQUE_PTR) && !defined(CATCH_CONFIG_CPP11_UNIQUE_PTR) && \
    !defined(CATCH_CONFIG_NO_CPP11)
...
void registerTestCaseFunction(TestFunction function, SourceLineInfo const& lineInfo,
                              NameAndDesc const& nameAndDesc);
...
#define INTERNAL_CATCH_TEST_CASE_METHOD(ClassName, ...)                            \
	...
	Catch::AutoReg INTERNAL_CATCH_UNIQUE_NAME(autoRegistrar)(                        \
	    &INTERNAL_CATCH_UNIQUE_NAME(____C_A_T_C_H____T_E_S_T____)::test, #ClassName, \
...
#define INTERNAL_CATCH_TEST_CASE_METHOD(ClassName, TestName, Desc)                 \
	...
	Catch::AutoReg INTERNAL_CATCH_UNIQUE_NAME(autoRegistrar)(                        \
	    &INTERNAL_CATCH_UNIQUE_NAME(____C_A_T_C_H____T_E_S_T____)::test, #ClassName, \
	    Catch::NameAndDesc(TestName, Desc), CATCH_INTERNAL_LINEINFO);                \
	}                                                                                \
	void INTERNAL_CATCH_UNIQUE_NAME(____C_A_T_C_H____T_E_S_T____)::test()
...
template <typename ExpressionT>
Generic::AllOf<ExpressionT> Matcher<ExpressionT>::operator&&(
    Matcher<ExpressionT> const& other) const {
...
template <typename ExpressionT>
Generic::AnyOf<ExpressionT> Matcher<ExpressionT>::operator||(
    Matcher<ExpressionT> const& other) const {
...
inline Impl::StdString::Equals Equals(
...
inline Impl::StdString::Contains Contains(
...
#else
	template <typename T> static std::string convert(T const&) { return unprintableString; }
#endif
};
...
#define INTERNAL_CATCH_TEST(expr, resultDisposition, macroName)                   \
	do {                                                                            \
		Catch::ResultBuilder __catchResult(macroName, CATCH_INTERNAL_LINEINFO, #expr, \
		                                   resultDisposition);                        \
		try {                                                                         \
	...
	} while (Catch::isTrue(false && (expr)))  // expr here is never evaluated at runtime but
...
template <typename T> class CompositeGenerator {
 public:
	...
	operator T() const {
		...
		return T();  // Suppress spurious "not all control paths return a value" warning in
		             // Visual Studio - if you know how to fix this please do so
	}
...
#define INTERNAL_CATCH_TRANSLATE_EXCEPTION(signature)                                \
	static std::string INTERNAL_CATCH_UNIQUE_NAME(catch_internal_ExceptionTranslator)( \
	    signature);                                                                    \
	namespace {                                                                        \
...
struct TestCaseInfo {
	...
	TestCaseInfo(std::string const& _name, std::string const& _className,
	             std::string const& _description, std::set<std::string> const& _tags,
...
class Text {
 public:
	...
	inline friend std::ostream& operator<<(std::ostream& _stream, Text const& _text) {
		for (Text::const_iterator it = _text.begin(), itEnd = _text.end(); it != itEnd;
		     ++it) {
...
template <typename ConfigT> class CommandLine {
	...
	class ArgBuilder {
	 public:
		...
		// Bind a free function taking a single argument - the object to operate on (no
		// placeholder string required)
		template <typename C> void bind(void (*unaryFunction)(C&)) {
			m_arg->boundField = new Detail::BoundUnaryFunction<C>(unaryFunction);
		}

		// Bind a free function taking a single argument - the object to operate on (requires
 ...
 public:
	...
	void optUsage(std::ostream& os, std::size_t indent = 0,
		...
		for (it = itBegin; it != itEnd; ++it) {
			...
			Detail::Text desc(it->description,
...
inline std::vector<TestCase> sortTests(IConfig const& config,
...
void registerTestCase(ITestCase* testCase, char const* classOrQualifiedMethodName,
                      NameAndDesc const& nameAndDesc, SourceLineInfo const& lineInfo) {
	getMutableRegistryHub().registerTest(
...
class ReporterRegistry : public IReporterRegistry {
 public:
	...
	virtual IStreamingReporter* create(
	    std::string const& name, Ptr<IConfig const> const& config) const CATCH_OVERRIDE {
...
ResultBuilder::ResultBuilder(char const* macroName, SourceLineInfo const& lineInfo,
                             char const* capturedExpression,
                             ResultDisposition::Flags resultDisposition,
...
template <typename T> class ReporterRegistrar {
	class ReporterFactory : public SharedImpl<IReporterFactory> {
		...
		// In fact, ideally, please contact me anyway to let me know you've hit this - as I
		// have
		// no idea who is actually using custom reporters at all (possibly no-one!).
		// The new interface is designed to minimise exposure to interface changes in the
		// future.
		virtual IStreamingReporter* create(ReporterConfig const& config) const {
			return new T(config);
		}
...
struct ConsoleReporter : StreamingReporterBase {
 ...
 private:
	...
	void printSummaryRow(std::string const& label, std::vector<SummaryColumn> const& cols,
	                     std::size_t row) {
		for (std::vector<SummaryColumn>::const_iterator it = cols.begin(); it != cols.end();
		     ++it) {
...
#define CATCH_REQUIRE_FALSE(expr)                                                   \
	INTERNAL_CATCH_TEST(                                                              \
	    expr, Catch::ResultDisposition::Normal | Catch::ResultDisposition::FalseTest, \
	    "CATCH_REQUIRE_FALSE")
...
#define CATCH_REQUIRE_THROWS_AS(expr, exceptionType)                              \
	INTERNAL_CATCH_THROWS_AS(expr, exceptionType, Catch::ResultDisposition::Normal, \
...
#define CATCH_REQUIRE_NOTHROW(expr)                                                \
	INTERNAL_CATCH_NO_THROW(expr, Catch::ResultDisposition::Normal, "CATCH_REQUIRE_" \
	                                                                "NOTHROW")
...
#define CATCH_CHECKED_IF(expr)                                                          \
	INTERNAL_CATCH_IF(expr, Catch::ResultDisposition::ContinueOnFailure, "CATCH_CHECKED_" \
	                                                                     "IF")
...
#define CATCH_CHECK_THROWS_WITH(expr, matcher)                                      \
	INTERNAL_CATCH_THROWS(expr, Catch::ResultDisposition::ContinueOnFailure, matcher, \
...
#define CHECK_THAT(arg, matcher)                                                 \
	INTERNAL_CHECK_THAT(arg, matcher, Catch::ResultDisposition::ContinueOnFailure, \
	                    "CATCH_CHECK_THAT")
...
#define CATCH_WARN(msg)                         \
	INTERNAL_CATCH_MSG(Catch::ResultWas::Warning, \
	                   Catch::ResultDisposition::ContinueOnFailure, "CATCH_WARN", msg)
...
#define CATCH_FAIL(...)                                 \
	INTERNAL_CATCH_MSG(Catch::ResultWas::ExplicitFailure, \
	                   Catch::ResultDisposition::Normal, "CATCH_FAIL", __VA_ARGS__)
#define CATCH_SUCCEED(...)                                                              \
	INTERNAL_CATCH_MSG(Catch::ResultWas::Ok, Catch::ResultDisposition::ContinueOnFailure, \
...
#define CHECK(expr) \
	INTERNAL_CATCH_TEST(expr, Catch::ResultDisposition::ContinueOnFailure, "CHECK")
